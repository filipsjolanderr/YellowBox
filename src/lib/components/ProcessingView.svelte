  ...
  let allZipsValid = $derived(
    session.selectedZips.length > 0 &&
      session.selectedZips.every(
        (z: string) => session.zipValidity[z] === "valid",
      ),
  );
...
<div class="flex flex-col h-full w-full bg-background overflow-hidden">
  <main class="flex-1 overflow-hidden relative">
    {#if viewState() === "setup"}
      <div
        class="h-full w-full flex flex-col overflow-y-auto p-12 gap-10"
        in:fade
      >
        ...
        <div class="flex flex-col gap-8">
          ...
          <section class="flex flex-col gap-3">
            ...
            {#if !session.selectedOutput}
              <button
                ...
                class="group flex items-center justify-between gap-4 px-6 py-8 bg-card border border-dashed border-border/70 rounded-2xl hover:border-primary/60 hover:bg-primary/5 active:bg-primary/7 transition-all duration-300 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 cursor-pointer"
                in:fade
              >
                <div class="flex items-center gap-4">
                  <div
                    class="h-12 w-12 rounded-2xl bg-muted/70 flex items-center justify-center group-hover:bg-primary/10 transition-colors ring-1 ring-border/50"
                  >
                    <FolderOpen
                ...
                <Plus
                  class="h-5 w-5 text-muted-foreground group-hover:text-primary transition-colors shrink-0"
                />
              </button>
            {:else}
              <button
                onclick={onSelectOutput}
                class="w-full flex items-center justify-between gap-4 px-6 py-5 bg-card rounded-2xl border border-border/60 shadow-sm ring-1 ring-primary/5 cursor-pointer hover:bg-primary/5 transition-colors text-left"
                in:slide
              >
                <div class="flex items-center gap-4 min-w-0">
                  ...
                  <div class="flex flex-col min-w-0">
                    <span class="text-base font-bold text-foreground truncate"
                      >{getFileName(session.selectedOutput)}</span
                    >
                    <span
                      class="text-xs text-muted-foreground truncate font-mono mt-1"
                      >{session.selectedOutput}</span
                    >
                  </div>
                </div>
                <Button
                  ...
                  size="sm"
                  onclick={onSelectOutput}
                  class="text-xs font-medium hover:text-primary h-9 px-4 shrink-0"
                >
                  Change
                </Button>
              </button>
          ...
          <section class="flex flex-col gap-3">
            <div
              class="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-muted-foreground"
            >
              <div class="flex items-center gap-2">
                <ImageIcon class="h-3.5 w-3.5" />
                Select your memories ZIPs
              </div>
              <Badge
                variant="outline"
            ...
            <div
              class="flex flex-col border rounded-2xl overflow-hidden bg-muted/5"
            >
                  ...
                  >
                    <div class="flex items-start gap-3">
                      ...
                      <div class="min-w-0">
                        ...
                        <ul
                          class="mt-2 space-y-1 text-xs font-mono text-muted-foreground"
                        >
                ...
                {:else}
                  <div class="grid grid-cols-1 gap-2.5">
                    {#each zipStats() as zip (zip.path)}
                      <div
                        class="group relative flex items-center gap-3 p-3 bg-card border rounded-xl shadow-sm hover:shadow-md transition-all"
                        in:slide
                      >
                        <div
                          class="h-8 w-8 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0 ring-1 ring-primary/10"
                        >
                        ...
                        <div class="flex flex-col min-w-0 pr-8">
                          ...
                          >
                          <div
                            class="flex items-center gap-1 mt-0.5 text-[9px] font-bold"
                            in:fade
                          >
                  ...
                  <Button
                    variant="outline"
                    onclick={onAddZip}
                    class="w-full border-dashed py-4 h-auto text-xs font-bold uppercase tracking-tighter gap-2"
                  >
                    <Plus class="h-4 w-4" />
                    Add More
                  </Button>
        ...
        <div class="flex flex-col items-center text-center gap-4">
          ...
          <div class="mt-8 flex flex-col items-center gap-2">
            <h2 class="text-3xl font-black tracking-tight">
              {session.progressPercentage.toFixed(1)}%
            </h2>
            <div
    ...
    {:else if viewState() === "finished"}
      ...
      >
        ...
        <div class="flex flex-col gap-8 max-w-xl mx-auto w-full">
          <div
            class="p-8 bg-primary/5 rounded-3xl border border-primary/10 flex flex-col justify-center gap-4 text-center items-center"
          >
